...
import com.brymm.brymmapp.local.AnadirPlatoActivity;
...
public class ListaPlatosFragment extends Fragment implements Lista {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View root = inflater.inflate(R.layout.fragment_lista_platos, container,
				false);
		return root;
	}
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		inflater.inflate(R.menu.context_menu_plato, menu);
	...
	public boolean onContextItemSelected(MenuItem item) {
		AdapterContextMenuInfo info = (AdapterContextMenuInfo) item
				.getMenuInfo();
		PlatoAdapter platoAdapter = (PlatoAdapter) lvPlatos.getAdapter();
		Plato plato = platoAdapter.getItem(info.position);

		switch (item.getItemId()) {
		case R.id.contextMenuPlatoBorrar:
			BorrarPlato bp = new BorrarPlato();
			bp.execute(plato.getIdPlato());
			return true;

		case R.id.contextMenuPlatoModificar:
			mostrarModificarPlato(plato);
			return true;
		}
	...
	private void inicializar() {
		lvPlatos = (ListView) getActivity().findViewById(
				R.id.listaPlatosLvLista);
		btAnadir = (Button) getActivity()
				.findViewById(R.id.listaPlatosBtanadir);
	...
	public void actualizarLista() {
		...
		platos = gestor.obtenerPlatos();
		gestor.cerrarDatabase();

		PlatoAdapter platoAdapter = new PlatoAdapter(getActivity(),
				R.layout.plato_item, platos);

		lvPlatos.setAdapter(platoAdapter);
	}

	private void mostrarNuevoPlato() {
		if (mDualPane) {
			AnadirPlatoFragment anadirFragment;

			// Make new fragment to show this selection.
			anadirFragment = new AnadirPlatoFragment();
			...
			Intent intent = new Intent(getActivity(), AnadirPlatoActivity.class);
			startActivityForResult(intent, MenusActivity.REQUEST_ANADIR_PLATO);
		}

	}

	private void mostrarModificarPlato(Plato plato) {
		if (mDualPane) {
			AnadirPlatoFragment anadirFragment;
			...
			bundle.putParcelable(AnadirPlatoFragment.EXTRA_PLATO, plato);
			...
			Intent intent = new Intent(getActivity(), AnadirPlatoActivity.class);
			intent.putExtra(AnadirPlatoFragment.EXTRA_PLATO, plato);
			startActivityForResult(intent, MenusActivity.REQUEST_ANADIR_PLATO);
		}

	}

	private JSONObject borrarPlato(int idPlato) {
		String respStr;
		JSONObject respJSON = null;

		try {
			String url = LoginActivity.SITE_URL
					+ "/api/menus/borrarPlato/format/json";
	...
	public class BorrarPlato extends AsyncTask<Integer, Void, Resultado> {
		...
		protected Resultado doInBackground(Integer... params) {
			JSONObject respJSON = null;
			this.idPlato = params[0];
			String mensaje = "";
			Resultado res = null;
			try {
				respJSON = borrarPlato(this.idPlato);
				...
				if (respJSON != null) {
					...
					if (operacionOk) {
						...
						GestionPlato gestor = new GestionPlato(getActivity());
						gestor.borrarPlato(this.idPlato);
						gestor.cerrarDatabase();
